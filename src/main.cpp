...
int main() {
    std::cout << "=== TEST CALCOLO BAC ===\n\n";

    double userWeight = 70.0;
    Breathalyzer breathalyzer(userWeight);

    // 2 bicchieri vino rosso 125ml, 12%, ingeriti un'ora fa
    Drink wine("vino rosso", 125.0, 12.0, 2, 1.0);
    breathalyzer.addDrink(wine);

    // 1 bicchiere di birra, 200ml, 5%, ingerito 2 ore fa
    Drink beer("birra", 200.0, 5.0, 1, 2.0);
    breathalyzer.addDrink(beer);

    // 2 porzioni di pasta al pomodoro, 300g, mangiate 2 ore fa
    Meal meal1("Pasta pomodoro", 300.0, 2.0);
    breathalyzer.addMeal(meal1);
